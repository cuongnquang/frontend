...
export function DoctorChatWindow({ conversation, onBack }: ChatWindowProps) {
  const {
    messages,
  ...
  const getParticipantName = (participant: any) => {
    return participant?.Patient?.full_name || participant?.full_name || 'Unknown';
  };
  ...
  return (
    <div className="w-full h-[600px] flex flex-col bg-white">
      {/* Header */}
      <div className="p-4 border-b bg-gradient-to-r from-green-600 to-emerald-600 text-white flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-lg">Bệnh nhân {getParticipantName(otherUser)}</h3>
              <p className="text-xs text-green-100">{isOnline ? 'Đang hoạt động' : 'Ngoại tuyến'}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        {loading ? (
          ...
          conversationMessages.map((message) => {
            ...
            const bubbleClass = isSender
              ? 'bg-green-500 text-white rounded-tl-lg rounded-b-lg max-w-[70%]'
                  ...
                  {message.isDeleted ? (
                    ...
                    <>
                      ...
                      <div className="flex items-center justify-between gap-2 mt-1">
                        <p className={`text-xs ${isSender ? 'text-green-100' : 'text-gray-500'}`}>
      ...
      <div className="p-4 border-t bg-white">
        {editingId && (
          <div className="mb-2 p-2 bg-green-50 rounded flex items-center justify-between">
            <p className="text-sm text-green-900">Đang sửa tin nhắn...</p>
            <button
              onClick={() => {
                setEditingId(null);
                setMessageContent('');
        ...
        <div className="flex gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 p-3 border border-gray-200 rounded-lg resize-none text-black outline-none focus:ring-2 focus:ring-green-500"
          />
          <button
